        ...
        <Toolbar sx={{ justifyContent: 'space-between', px: { xs: 1, sm: 2 } }}>
          ...
          <Box component={RouterLink} to="/" sx={{ display: 'flex', alignItems: 'center', textDecoration: 'none', color: 'inherit' }}>
            <Box
              ...
              src="/logo-img.jpg"
              alt="Logo"
              sx={{ height: { xs: '32px', sm: '40px' }, mr: '8px', borderRadius: '4px' }}
            />
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: { xs: 0.5, sm: 1.5 } }}>
            ...
            <Button component={RouterLink} to="/book" variant="contained"
              sx={{ bgcolor: 'primary.main', fontWeight: '900', borderRadius: '20px', px: { xs: 1.5, sm: 3 }, fontSize: { xs: '0.75rem', sm: '0.875rem' }, '&:hover': { bgcolor: '#b71c1c' }, whiteSpace: 'nowrap', minWidth: 'auto' }}
            >
              BOOK NOW
            </Button>
        ...
        <List sx={{ pt: 5 }}>
          ...
          <ListItem disablePadding>
            <ListItemButton component={RouterLink} to="/book" onClick={() => setMobileOpen(false)}>
              <ListItemText primary="BOOK NOW" sx={{ color: theme.palette.primary.main, textAlign: 'center', '& span': { fontWeight: 'bold' } }} />
            </ListItemButton>
          </ListItem>
          {user && (
            <ListItem disablePadding>
          ...
          <ListItem disablePadding sx={{ mt: 5 }}>
            {!user ? (
              <ListItemButton component={RouterLink} to="/login" onClick={() => setMobileOpen(false)} sx={{ mx: 4, border: '1px solid white', borderRadius: '10px' }}>
